...
const InputVariants = cva(
  "flex flex-row gap-4 items-center justify-center p-3 py-1 rounded-md text-md font-medium border outline-none",
  {
    variants: {
      variant: {
  ...
  ({ children, className, variant, preIcon, sufIcon, name, ...props }, ref) => {
    ...
    return (
      <UseMemo dependencies={[props.value]}>
        ...
        <ErrorMessage name={name} className="text-xs text-red-800" />
      </UseMemo>
    )
  }
)
